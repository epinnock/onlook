...
const RUNTIME_HASH = 'runtime-hash-eval-errors';
const RUNTIME_MANIFEST_URL = `exp://preview.test/manifest/${RUNTIME_HASH}`;
const RUNTIME_EVAL_ERROR = 'Unexpected token <';
...
test.describe('Mobile preview eval errors', () => {
    test('runtime eval errors surface in the preview status modal', async ({
        page,
    }) => {
        ...
        const expectedErrorMessage = `Mobile preview runtime error: ${RUNTIME_EVAL_ERROR}`;
        ...
        }, JSON.stringify({ type: 'evalError', error: RUNTIME_EVAL_ERROR }));

        const errorState = page.getByTestId('qr-status-error');
        await expect(errorState).toBeVisible();
        await expect(errorState).toContainText(expectedErrorMessage);

...
Branch runtime modes:
Some branches in this project may run in browser-preview mode (the ExpoBrowser provider). On those branches:
- Shell tools (bash_read, bash_edit) and arbitrary terminal_command calls return an error code "PROVIDER_NO_SHELL". When you see that, switch to file operation tools (read_file, write_file, list_files) and the in-process search tools (glob, grep) instead.
- terminal_command still works for: \`npm install <pkg>\`, \`npm uninstall <pkg>\`, \`npm run dev\`, \`npm run build\`, and the equivalent bun/yarn/expo commands. Use those for package management and dev server restarts.
- The typecheck tool is not yet available on browser-preview branches; rely on the bundler error overlay instead.
- All other functionality (file ops, glob, grep, git status) is fully available — it just runs locally instead of in a shell.

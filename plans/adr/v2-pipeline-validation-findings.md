...
- Screenshot evidence (permanent):
  ...
  - `plans/adr/assets/v2-pipeline/post-g-hello.png` — post-Phase-G E2E mount (2026-04-23): cold-launched via `onlook://launch?session=test&relay=...` deep link on the REBUILT app. Full pipeline exercised via COMMITTED code (not mock-relay append): `App.tsx` Linking listener → `qrToMount` parse/manifest/bundle/mount → `OnlookRuntime.mountOverlay` → Hermes eval → `globalThis.renderApp` (pinned, subscribable) → `OverlayHost` subscriber notification → React tree mount. Log trail: `[qrToMount] stage=parse ok` → `stage=manifest ok` → `stage=bundle ok bytes=10326` → `stage=mount OnlookRuntime.mountOverlay() bytes=10326 relayHost=192.168.0.17` → `[v2-demo] appended snippet running` → `[B13] onlookMount invoked rootTag=11 sessionId=test` → `stage=mount mountOverlay() returned` → `[v2-demo] hello renderApp returned`.
  - `plans/adr/assets/v2-pipeline/post-g-updated.png` — post-Phase-G in-place update (2026-04-23): second `renderApp` push 7s later with different colour + text, OverlayHost re-renders via subscribable pump. Log trail: `[v2-demo] updated renderApp returned`.

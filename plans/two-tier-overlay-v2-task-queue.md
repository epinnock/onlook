...
| 89–94 | partial | Deprecation warnings added to `wrapOverlayCode` + `pushOverlay` (task #16). Feature flag `overlay-v1` shipped (task #15). Migration path documented 2026-04-23 in `plans/adr/phase-11-legacy-overlay-migration.md` — 4-phase rollout (11a flag-gate parallel v1 path in `two-tier.ts`, 11b default-flip, 11c delete legacy after 7-day soak, 11d schema cleanup). Removal of legacy paths itself still pending, but the sequence + risk model is now explicit. |
...
| 98–100 | done (primitive) | Negative/perf/size gates — `checkOverlaySize(bundle, { softCap?, hardCap? })` in `packages/browser-bundler/src/check-overlay-size.ts` returns `{ status: 'ok' \| 'warn-soft' \| 'fail-hard', bytes, softCap, hardCap, message }`. Pure function with no Bun/Node-specific globals, safe for Workers. Handles both string (UTF-8 encoded) and Uint8Array inputs. 15 bun tests cover: ok/warn-soft/fail-hard transitions at exact boundaries (≥soft-cap is ok, >soft-cap warns, >hard-cap fails), UTF-8 byte counting for multi-byte chars, option overrides for tighter limits, soft≥hard misconfiguration throw, message formatting for each status. Exported from the package root so CI/editor callers can gate uploads. Size-gate boundary tests in `wrap-overlay-v1.test.ts` (5 tests) remain the wrapper-side coverage. CI wiring (GitHub workflow gate) not included — left to the operator to compose. |

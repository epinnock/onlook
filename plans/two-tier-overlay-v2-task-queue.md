...
| 85 | pending | Tap-to-source phone → editor cursor jump — JS-layer wiring shipped end-to-end. Phone side: `onlookSelectReceiver.ts` + AppRouter tap bridge dispatch on tap. Editor side: `wireOnlookSelectToIdeManager` (commit `9eda7ddb`) + new `IdeManager.openCodeLocation(fileName, line, col)` method drive the existing `_codeNavigationOverride` MobX state, which the `useCodeNavigation` hook applies to CodeMirror's `EditorView`. Production wire-up at `apps/web/client/src/app/project/[id]/_components/main.tsx` runs `wireOnlookSelectToIdeManager(editorEngine.ide)` in a `useEffect` (commit `47a01022`). Audit caught a stale Monaco-shaped helper (`wireCursorJump` + `monacoCursorJump`) that never matched this codebase's CodeMirror editor — deleted in commit `2be936c8`. **2026-04-25:** audit caught a second gap — the bundler did NOT inject `__source` props on JSX (esbuild's automatic JSX runtime doesn't emit `__source`, unlike Babel's `transform-react-jsx-source`). `createJsxSourcePlugin` was shipped + tested under task #34/#84 but was never wired into `bundleBrowserProject`. Without `__source` on rendered JSX, the phone-side `TapHandler` reads `undefined` off `props.__source` and the dispatched `onlook:select` message has no file/line/col coordinates → editor receives the message but cannot jump. Wired in `bundle.ts` (gated on new `injectJsxSource?: boolean` option default false) + always-on for `two-tier.ts` overlay path (both v1 and legacy branches). 2 new bundle.test.ts assertions for plugin order. Full device flow (mobile-client side) remains a device-only Maestro task on MC4.18. |

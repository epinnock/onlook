...
| 68 | partial | Tests: 26 green (up from 10 on 2026-04-23) across `assets-inline.test.ts` (13) + `assets-r2.test.ts` (13). New coverage: all registered image MIME types (jpg/jpeg/webp/gif/avif/bmp/ico), all font MIME types (ttf/otf/woff/woff2), case-insensitive extension inference, deterministic data URL for same bytes, path normalization (backslash + leading slash), exact-threshold byte inlining (boundary `length === maxInlineBytes`), `defaultAssetKey` content-addressing contract (4 tests), `createImmutableAssetUrl` invariants (4 tests), R2 exact-threshold boundary (prefers inline). Per-class coverage (`.svg?url`/`.svg?raw`, audio/video, JSON descriptors, binary bytes, missing-asset path) lands as each #55–#67 slot fills. |

...
| 85 | pending | Tap-to-source phone → editor cursor jump — JS-layer wiring shipped end-to-end. Phone side: `onlookSelectReceiver.ts` + AppRouter tap bridge dispatch on tap. Editor side: `wireOnlookSelectToIdeManager` (commit `9eda7ddb`) + new `IdeManager.openCodeLocation(fileName, line, col)` method drive the existing `_codeNavigationOverride` MobX state, which the `useCodeNavigation` hook applies to CodeMirror's `EditorView`. Production wire-up at `apps/web/client/src/app/project/[id]/_components/main.tsx` runs `wireOnlookSelectToIdeManager(editorEngine.ide)` in a `useEffect` (commit `47a01022`). Audit caught a stale Monaco-shaped helper (`wireCursorJump` + `monacoCursorJump`) that never matched this codebase's CodeMirror editor — deleted in commit `2be936c8`. **2026-04-25:** audit caught two more gaps closing the chain end-to-end. (a) The bundler did NOT inject `__source` props on JSX (esbuild's automatic JSX runtime doesn't emit `__source`, unlike Babel's `transform-react-jsx-source`). `createJsxSourcePlugin` was shipped + tested under task #34/#84 but was never wired into `bundleBrowserProject`. Wired in `bundle.ts` (gated on new `injectJsxSource?: boolean` option, default false) + always-on for `two-tier.ts` overlay path (both v1 and legacy branches). 2 new bundle.test.ts assertions for plugin order. Commit `5da582fe`. (b) Even with `__source` injected and the phone dispatching valid `SelectMessage` shapes, the editor's `useRelayWsClient` handlers object in `use-mobile-preview-status.tsx` only set `onOverlayAck` + `onError` — no `onSelect`. `dispatchOnlookSelect` (the in-process pub-sub publisher) was exported but never invoked from any production caller, so parsed `SelectMessage` instances were silently dropped at `handlers.onSelect?.(msg)`. Wired in commit `3b18789d` — one-line addition to the handlers object. Full chain now: JSX __source → tap handler → WS → relay-events → dispatchOnlookSelect → registered handler → IdeManager.openCodeLocation → CodeMirror cursor jump. Full device flow (mobile-client side) remains a device-only Maestro task on MC4.18. |

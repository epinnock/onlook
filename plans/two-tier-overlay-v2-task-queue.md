...
| 35 | partial | wrapOverlayV1 accepts a `sourceMap?: string` passthrough; editor's `push-overlay.ts` packs it into `OverlayMeta.sourceMapUrl` when present. Receive side: `apps/web/client/src/services/expo-relay/overlay-sourcemap.ts` ships `fetchOverlaySourceMap` + `resolveOverlayFrame` + `decorateRuntimeErrorWithSourceMap` for decoding runtime error frames through the map. Test coverage at `__tests__/overlay-sourcemap.test.ts`. **Production wiring gap caught 2026-04-25:** `decorateRuntimeErrorWithSourceMap` has no production caller (`grep -rn 'decorateRuntimeErrorWithSourceMap' apps/web/client/src` returns only its own definition). The receiver chain at `relay-events.ts::onError(msg)` and `relay-ws-client.ts`'s buffered `messages` stream pass `onlook:error` through verbatim without source-map decoration, so the editor's Console tab and error UI today show raw bundle frames (`bundle.js:NNN:NN`) instead of mapped source locations. Wire-up needs: (a) per-session `lastOverlayMeta.sourceMapUrl` tracking in `useMobilePreviewStatus` or a sibling hook, (b) on `onlook:error` arrival, look up the URL + `fetchOverlaySourceMap(url)` (cache by URL since identical overlays share map URLs), (c) `decorateRuntimeErrorWithSourceMap(error, map)` before forwarding to dev-panel tabs. Also: the original "Monaco cursor jumps" copy in the row was incorrect — this codebase uses CodeMirror's `EditorView` (see project memory `editor-uses-codemirror`); the cursor jump fires through `IdeManager.openCodeLocation` (post-2026-04-25) once the resolved source frame is available. |

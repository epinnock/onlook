...
| 47 | partial | `createInMemoryPureJsCache()` ships. Durable cache (KV/R2) layer deferred to Phase 9 editor integration — the editor's dev-loop can hot-path through memory today. |
| 48 | partial | `resolvePureJsModule(artifact, specifier)` composes the bare-import resolver. Editor-side fetch plumbing from cache → bundler lands with Phase 9 package install flow. |
...
| 63 | partial | Metadata: sha256 hash + MIME type + size in bytes shipped across `defaultAssetKey`, `assets-inline` plugin, and `assets-resolve` plugin. SVG viewBox extracted via `extractSvgViewBox` (regex-based, handles single+double-quoted attrs). Font `family` derived from filename. Image dimensions / scale / font-weight / font-style hints remain Phase 7 polish — require binary parsing (PNG IHDR / JPEG SOF / font tables). |
...
| 68 | partial | Tests: 45 green (10 → 26 → 45 across 2026-04-23) across `assets-inline.test.ts` (13) + `assets-r2.test.ts` (13) + `assets-raw-text.test.ts` (19). `?raw` / raw-text paths now covered. Missing-asset path covered for the raw-text plugin (returns `undefined` so esbuild falls through). Per-class coverage still pending for audio/video (#59), JSON descriptors (#60), binary bytes (#62), Metro-registry emulation (#67). |
...
| 84 | pending | `__source` injection — tracked as #34 (requires a Babel plugin in the overlay build path; deferred with inspector roll-out). |
...
| 95 | partial | Unit tests green: ABI schemas, wrap-overlay (27 tests), preflight (7 tests), runtime (18 tests), platform-resolver (13 tests), package-resolver (13 tests, including `react-native`/`exports`/`main`/`module` fields), pure-js-package + pure-js-fixtures (12 tests). Full browser-bundler suite: 161 pass across 25 files. Asset-resolver tests remain for Phase 7 (#54–68). |
...
| 98–100 | partial | Negative/perf/size gates — instrumentation present (`sizeWarning` + `OVERLAY_SIZE_SOFT_CAP`/`OVERLAY_SIZE_HARD_CAP` in wrapOverlayV1). Size-gate boundary coverage added 2026-04-23 via 5 new tests in `wrap-overlay-v1.test.ts` (well-below soft, just-below-soft with 1 KB margin, mid-range warning, above-soft warning, SOFT<HARD sanity). Full suite: 27 pass. CI-enforcement step (fail build when overlay > hard cap) still pending — needs `.github/workflows/` gate decision. |

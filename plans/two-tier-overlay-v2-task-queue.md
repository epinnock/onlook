...
| 23 | done | Validated on Mac mini Xcode 16.4 + iOS 18.6 simulator 2026-04-23 (commit `2cdd2c07`). `OnlookRuntimeInstaller.cpp::install()` executes at app launch via `index.js:104-117` `TurboModuleRegistry.get(...).install()`. Log trace: `[onlook-runtime-direct] installHostObject entered` + `nativeLoggingHook installer completed` + Hermes JSI host-object installation. Screenshot: `plans/adr/assets/v2-pipeline/post-h-native-sim.png`. |
| 24 | done | `OnlookRuntime_runApplication.cpp` wired through the installer's host-object property chain (`OnlookRuntime::get()` → `makeHostMethod()` → `runApplicationImpl()`). Compiles + links clean on Xcode 16.4 x86_64 + arm64 (verified via xcodebuild BUILD SUCCEEDED 2026-04-23). |
| 25 | done | `OnlookRuntime_reloadBundle.cpp` wired the same way via `OnlookRuntime::reloadBundle()` → `reloadBundleImpl()`. Compiles + links clean on Xcode 16.4 universal-sim build 2026-04-23. `mountOverlay` also inline at `OnlookRuntime.cpp:176-201`. |
...
| 35 | partial | wrapOverlayV1 accepts a `sourceMap?: string` passthrough; editor's `push-overlay.ts` packs it into `OverlayMeta.sourceMapUrl` when present. Receive side: `apps/web/client/src/services/expo-relay/overlay-sourcemap.ts` ships `fetchOverlaySourceMap` + `resolveOverlayFrame` + `decorateRuntimeErrorWithSourceMap` for decoding runtime error frames through the map. Test coverage at `__tests__/overlay-sourcemap.test.ts`. **Production wiring gap caught 2026-04-25; primitives shipped, final wire-up has architectural choice point:** existing `decorateRuntimeErrorWithSourceMap` operated on `OnlookRuntimeError` shape only — extended with `decorateErrorMessageWithSourceMap` sibling for the WS shape (`0b09549f`). Memoizing cache + double-delivery middleware shipped in `9bbb1278` (`createSourceMapCache` + `withSourceMapDecoration`). Composition at the use-site is now ONE call, BUT `withSourceMapDecoration`'s shape (sync undecorated + async decorated double-delivery via `(ErrorMessage)=>void` callback) is correct for stream/telemetry consumers; the dev panel's buffer-based path (`relay-ws-client.ts::pushMessage` → `snapshot().messages`) doesn't natively support in-place updates, so wiring decoration there would either (a) duplicate rows in the UI or (b) need a buffer-mutation primitive (`updateMessageById(id, decorated)`) added to `RelayWsClient`. Architectural choice — separately, the `lastOverlayMeta.sourceMapUrl` resolver also needs a state slot in `useMobilePreviewStatus` or a sibling hook (Phase 9 R2 upload is the prerequisite to populate it). Also: the original "Monaco cursor jumps" copy in the row was incorrect — this codebase uses CodeMirror's `EditorView` (see project memory `editor-uses-codemirror`); the cursor jump fires through `IdeManager.openCodeLocation` (post-2026-04-25) once the resolved source frame is available. |
...
| 85 | pending | Tap-to-source phone → editor cursor jump — JS-layer wiring shipped end-to-end. Phone side: `onlookSelectReceiver.ts` + AppRouter tap bridge dispatch on tap. Editor side: `wireOnlookSelectToIdeManager` (commit `9eda7ddb`) + new `IdeManager.openCodeLocation(fileName, line, col)` method drive the existing `_codeNavigationOverride` MobX state, which the `useCodeNavigation` hook applies to CodeMirror's `EditorView`. Production wire-up at `apps/web/client/src/app/project/[id]/_components/main.tsx` runs `wireOnlookSelectToIdeManager(editorEngine.ide)` in a `useEffect` (commit `47a01022`). Audit caught a stale Monaco-shaped helper (`wireCursorJump` + `monacoCursorJump`) that never matched this codebase's CodeMirror editor — deleted in commit `2be936c8`. **2026-04-25:** audit caught three gaps. **Editor-side now correct:** (a) The bundler did NOT inject `__source` props on JSX (esbuild's automatic JSX runtime doesn't emit `__source`, unlike Babel's `transform-react-jsx-source`). `createJsxSourcePlugin` was shipped + tested under task #34/#84 but was never wired into `bundleBrowserProject`. Wired in `bundle.ts` (gated on new `injectJsxSource?: boolean` option, default false) + always-on for `two-tier.ts` overlay path. Commit `5da582fe`. (b) `useRelayWsClient` handlers in `use-mobile-preview-status.tsx` set only `onOverlayAck` + `onError` — no `onSelect`. `dispatchOnlookSelect` (the in-process pub-sub publisher) was exported but never invoked from production, so parsed `SelectMessage` instances were silently dropped. Wired in commit `3b18789d`. **Mobile-client side has unimplemented prerequisites (multi-day, NOT shipped):** (c) `wireInspectorFlow` (the only function that instantiates `TapHandler` in non-test code) is exported from `flow/index.ts` but called nowhere — `OnlookRelayClient` itself is never instantiated in production (AppRouter Spike B WS bypasses it). (d) `findNodeAtPoint` (JS hit-tester referenced in `tapBridge.ts:14` as "MC4.2") does not exist anywhere in the codebase. Native tap events from `OnlookTapForwarder` reach JS as `{x, y}` via `startTapBridge`, but there's no path from `(x, y) → React node → __source` in production. Until (c) and (d) are implemented (multi-day mobile-client work that crosses MC4.2 + MC4.14 + MC4.16 + the canonical/Spike-B WS reconciliation), tap-to-source is editor-receiver-ready but does not actually deliver on a real device. Test/Maestro flow (MC4.18) is gated on these, not the reverse. |
...
| 89–94 | partial | Deprecation warnings added to `wrapOverlayCode` + `pushOverlay` (task #16). Feature flag `overlay-v1` shipped (task #15). Migration path documented 2026-04-23 in `plans/adr/phase-11-legacy-overlay-migration.md` — 4-phase rollout. **Phase 11a shipped 2026-04-23:** `two-tier.ts::sync()` now branches on `isMobilePreviewOverlayV1PipelineEnabled()`. With `NEXT_PUBLIC_MOBILE_PREVIEW_PIPELINE=overlay-v1` the pipeline emits a Hermes-safe ABI-v1 envelope via `wrapOverlayV1` and posts it as `OverlayUpdateMessage` (type='overlayUpdate', abi='v1', sessionId, source, assets={abi:'v1',assets:{}}, meta{overlayHash, entryModule, buildDurationMs}). Legacy branch preserved as default — Phase G simulator mount flow regresses only if the flag is flipped (Phase 11b soak work). 3 new dual-branch tests in `two-tier.test.ts` (12 → 12 pass overall): legacy branch emits OverlayMessage, v1 branch emits OverlayUpdateMessage with correct fields, overlayHash stable across identical rebuilds (cache-hit contract). sourceMap is omitted on the v1 branch pending Phase 9 R2 upload wiring. **Phase 11b prep shipped 2026-04-25:** the ADR's "Pre-flip check: phone runtime MUST be v1-capable" is now ENFORCED in code. End-to-end safety chain across mobile-client + cf-expo-relay + web-client (~18 commits on `fix/expo-go-runtime-load`): phone-side `buildPhoneAbiHello` + send-on-connect (legacy WS workaround in AppRouter `e6237acd` + canonical `OnlookRelayClient.abiHelloProvider` `b2bb133c`); relay-side `AbiHelloMessageSchema` routing + per-role `lastEditorHelloPayload`/`lastPhoneHelloPayload` replay-on-connect + `hmr.abi-hello.fwd`/`replay` structured logs (`80d3d54f`, `3e0f9074`); editor-side `startEditorAbiHandshake` armed via `RelayWsClient.editorCapabilities` + `getLastAbiCompatibility()` getter (`78b8e26e`, `aed09752`); `pushOverlayV1` opt-in `compatibility` gate fail-closes pre-network on `'unknown'` / `OnlookRuntimeError` with telemetry `category: 'compat-gate'` segmenting it from network/validation failures (`faf636f3`, `781dd6aa`); `TwoTierMobilePreviewPipeline.compatibilityProvider` deps slot threads it through (`8416f938`, `f7eca55a`); `useMobilePreviewStatus` captures handshake state in React + exposes `abiCompatibility`+`phoneHello` to `MobilePreviewDevPanelContainer` → `MobileDevPanel` (`ee1977fc`); `AbiCompatibilityIndicator` renders WAITING/OK/MISMATCH on screen (`f116074d`, `6acd82ae`); pre-existing `kind === 'two-tier'` literal-equality bug fixed so `env=overlay-v1` actually reaches the v1 push path (`b8ace3a2`); reconnect-replay resilience pinned (`9bde2490`); ADR updated (`3a2f9bc3`). +60 tests across the touched packages — typecheck clean. Operationally remaining: re-validate Phase G sim smoke (Xcode 16.1 unblock) and run the 7-day soak before flipping the env default. Phase 11c/11d still remain (gated on Phase 11b soak completion). |
...
| 98–100 | done | Negative/perf/size gates — full stack: (1) `checkOverlaySize(bundle, { softCap?, hardCap? })` in `packages/browser-bundler/src/check-overlay-size.ts` for editor/CI pre-push gating with 15 tests, (2) `wrap-overlay-v1.ts` enforces hard cap at build time with 5 boundary tests, (3) cf-expo-relay's `MAX_OVERLAY_BODY_BYTES=2MB` enforces hard cap at the `/push` body level (returns 413), (4) **2026-04-23:** cf-expo-relay's v1 handler now emits `sourceBytes` + `softCapExceeded` fields on the `hmr.push.v1` info log and emits a dedicated `hmr.push.v1.softcap` warn log when source size exceeds 512 KB soft cap. 2 new tests in `do-hmr-session.test.ts` verify under-cap/over-cap observability. Soft cap is advisory — oversized overlays still deliver. GitHub Actions workflow gate wiring remains the operator's decision. (5) **2026-04-25:** size-delta perf signal — `evaluateSizeDelta` in `perf-guardrails.ts` (commit `7a6a5272`) emits `size-grew` (warn @ ≥20%) / `size-shrunk` (info @ ≥10 KB drop) per-pipeline-instance after every successful push. Wires the previously-shipped-but-unwired `computeOverlaySizeDelta` (audit-pattern catch). 8 unit tests + 3 integration tests in `two-tier.test.ts`. Distinct from large-overlay's absolute cap — surfaces session-relative regressions across consecutive overlays. |
